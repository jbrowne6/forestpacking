...
int main(int argc, char* argv[]) {
	...
	outfile.open("experiments/experiment2/experiment2.csv");
				...
				for(int numCores = 1; numCores <= 32; numCores *= 2){	
					if(numBins >= numCores){
						for(int runNum = 0; runNum < numTimes; ++runNum){
							...
							outfile << std::fixed << "experiment2, Bin+, Higgs, MultiTree, " << treesPerBin << ", " << numDepth << ", " << diffMilli.count()<< ", " << numCores <<std::endl;
							std::cout << "percentRight does not match: " << percentRight << " : " << observations.returnPercentRight() << "\n";
							std::cout << std::fixed << "experiment2, Bin+, Higgs, MultiTree, " << treesPerBin << ", " << numDepth << ", " << diffMilli.count()<< ", " << numBins << ", " << numCores << std::endl;
						}
					}
				}
			}
					...
					for(int numCores = 1; numCores <= 32; numCores *= 2){	
						if(numBins >= numCores){
							for(int runNum = 0; runNum < numTimes; ++runNum){
								...
								stop_time = std::chrono::high_resolution_clock::now();
								diffMilli = stop_time - start_time;
								outfile << std::fixed << "experiment2, Bin+, MNIST, MultiTree, " << treesPerBin << ", " << numDepth << ", " << diffMilli.count()<< ", " << numCores << std::endl;
								std::cout << "percentRight does not match: " << percentRight << " : " << observations.returnPercentRight() << "\n";
								std::cout << std::fixed << "experiment2, Bin+, MNIST, MultiTree, " << treesPerBin << ", " << numDepth << ", " << diffMilli.count()<< ", " << numBins << ", " << numCores << std::endl;
							}
						}
					}
				}
						...
						for(int numCores = 1; numCores <= 32; numCores *= 2){	
							if(numBins >= numCores){
								for(int runNum = 0; runNum < numTimes; ++runNum){
									...
									stop_time = std::chrono::high_resolution_clock::now();
									diffMilli = stop_time - start_time;
									outfile << std::fixed << "experiment2, Bin+, Allstate, MultiTree, " << treesPerBin << ", " << numDepth << ", " << diffMilli.count()<< ", " << numCores << std::endl;
									std::cout << "percentRight does not match: " << percentRight << " : " << observations.returnPercentRight() << "\n";
									std::cout << std::fixed << "experiment2, Bin+, Allstate, MultiTree, " << treesPerBin << ", " << numDepth << ", " << diffMilli.count()<< ", " << numBins << ", " << numCores << std::endl;
								}
							}
						}
					}
