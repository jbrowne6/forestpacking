...
//    int batchSize = 10;
  //  int currentNode;
  int treesInBin;
    int predictions[batchSize][numOfBins][numOfClasses];
    ...
    int observationFeatures[batchSize][numFeatures] = {};

    int treesPerBin = numTreesInForest / numOfBins;
    int binRemainder = numTreesInForest % numOfBins;
    int multiCurrentNode[treesPerBin+1][batchSize];

    int currSample = 0;

    int numOfBatches = numObservations/batchSize;

    for(int currentBatchStart = 0; currentBatchStart < numOfBatches; currentBatchStart++){
        memset(predictions, 0, sizeof(predictions));
        for(int inBatchNum = 0; inBatchNum < batchSize; inBatchNum++){
        fin >> currentNumberFromFile;
        observationClasses[currSample++] = (int)currentNumberFromFile;
        for(int j=0; j < numFeatures; j++){
            fin >> currentNumberFromFile;
            observationFeatures[inBatchNum][j] = (int)currentNumberFromFile;
        }
        }

#pragma omp parallel for num_threads(2) schedule(dynamic) private(multiCurrentNode, treesInBin)
        for(int currBinNum = 0; currBinNum < numOfBins; currBinNum++){
            treesInBin = (currBinNum < binRemainder)? treesPerBin+1 : treesPerBin;
            // setup muliple current nodes to run through all trees in the bin in an interleaved fashion.
                for(int q = 0; q < treesInBin; q++){
            for(int qt = 0; qt < batchSize; qt++){
multiCurrentNode[q][qt] = q;
            }
            }
            // process top of node
            for(int m = 0; m < treeTopDepth; m++){
                for(int n = 0; n < treesInBin; n++){
                    for(int bs = 0; bs < batchSize; bs++){
                        if(forest[currBinNum][multiCurrentNode[n][bs]].isInternalNode()){
                            if(forest[currBinNum][multiCurrentNode[n][bs]].goLeft(observationFeatures[bs][forest[currBinNum][multiCurrentNode[n][bs]].returnFeature()])){
                                multiCurrentNode[n][bs] = forest[currBinNum][multiCurrentNode[n][bs]].returnLeftNode(); 
                            }else{
                                multiCurrentNode[n][bs] = forest[currBinNum][multiCurrentNode[n][bs]].returnRightNode(); 
                            }
                        }
                    }
                }
            }
            // make predictions one tree at a time after all tree top is finished.
            for(int k=0; k < treesInBin; k++){
               // currentNode = k;
               for(int bs = 0; bs < batchSize; bs++){
                while(forest[currBinNum][multiCurrentNode[k][bs]].isInternalNode()){
                    if(forest[currBinNum][multiCurrentNode[k][bs]].goLeft(observationFeatures[bs][forest[currBinNum][multiCurrentNode[k][bs]].returnFeature()])){
                        multiCurrentNode[k][bs] = forest[currBinNum][multiCurrentNode[k][bs]].returnLeftNode(); 
                    }else{
                        multiCurrentNode[k][bs] = forest[currBinNum][multiCurrentNode[k][bs]].returnRightNode(); 
                    }
                }
               
                predictions[bs][currBinNum][forest[currBinNum][multiCurrentNode[k][bs]].returnRightNode()]++;
               }
            }
        }
        ///////////////////////////////////////////////////////
        ...
        for(int p=1; p < numOfBins; p++){//TODO this should be SIMD
            ...
            for(int q=0; q < numOfClasses; q++){
                predictions[bs][0][q] += predictions[bs][p][q];
            }
        }
        }

        //reset currSample to pre read size
        currSample -= batchSize;
        for(int bs = 0; bs < batchSize; bs++){
        predictionClasses[currSample++] = returnClassPrediction(predictions[bs][0]);
        if(showAllResults){
            printf("observation%d actual %d predicted %d\n", currSample-1, observationClasses[currSample-1],predictionClasses[currSample-1]);
        }
        if(observationClasses[currSample -1] == predictionClasses[currSample-1]){
...
void topBinForestWBatch::createForestFromCSV(const std::string& forestCSVFileName){
