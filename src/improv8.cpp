...
#pragma omp parallel for num_threads(numCores) schedule (static) private(q, p, k, numberNotInLeaf, currentNode, predictions)
	for(int i = 0; i < observations.numObservations; i++){
...
//////////////////////////////////////////////////////
void improv8::makePredictionsMultiTreeAndObs(const inferenceSamples& observations, int numCores){
	int predictions[numOfClasses];
	int currentNode[forestRoots[0]->numOfTreesInBin];
	int numberNotInLeaf;
	int  p, k, q;
	int numCoresObs, numCoresBins;
...
#pragma omp parallel for num_threads(numCoresObs) schedule (static) 
	//#pragma omp parallel for num_threads(numCoresObs) schedule (static) private(q, numberNotInLeaf, currentNode)
	for(int i = 0; i < observations.numObservations; i++){
...
#pragma omp parallel for num_threads(numCoresBins) schedule (static) private(q, numberNotInLeaf, currentNode)
		for( k=0; k < numOfBins;++k){
			for( q=0; q<forestRoots[k]->numOfTreesInBin; q++){
				currentNode[q] = q;
				__builtin_prefetch(&forestRoots[k]->bin[currentNode[q]], 0, 3);
